 ...
 void initState() {
   ...
   Utils.check_connectivity().then((result){
     if(result) {
       ...
       pd.show();
       TrainingCareTakerServices.training_caretaker_by_page(token, pagenum, searchQuery).then((response){
         pd.dismiss();
         if(response!=null){
           setState(() {
                    ...
                    if(load_list['hasNext'] == true && pagenum >= 1 ) {
                      Utils.check_connectivity().then((result){
                        if(result) {
                          ProgressDialog pd = ProgressDialog(context, isDismissible: true, type: ProgressDialogType.Normal);
                          pd.show();
                          TrainingCareTakerServices.training_caretaker_by_page(
                            ...
                            child: FadeAnimation(2.0,
                              ListTile(
                                ...
                                title: Text(training_list != null
                                    ...
                                    : ''),
                                trailing:Text(training_list!=null?"Status: "+(get_status_by_id(training_list[index]['status'])).toString():'empty'),
