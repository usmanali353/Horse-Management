...
class _breeding_services_State extends State<breeding_services>{
  ...
  var breeding_services_list, load_list, pagelist, pageloadlist;
  int pagenum = 1;
  int total_page;
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((result){
      if(result) {
        ...
        pd.show();
        BreedingServicesJson.breeding_services_by_page(token, pagenum, searchQuery).then((response){
          pd.dismiss();
          if(response!=null){
            setState(() {
                    ...
                    if(load_list['hasNext'] == true && pagenum >= 1 ) {
                      Utils.check_connectivity().then((result){
                        if(result) {
                          ProgressDialog pd = ProgressDialog(context, isDismissible: true, type: ProgressDialogType.Normal);
                          pd.show();
                          BreedingServicesJson.breeding_services_by_page(
        ...
        BreedingServicesJson.breeding_services_by_page(token,pagenum,searchQuery).then((response){
          ...
          if(response!=null){
            setState(() {
              ...
              if(breeding_services_list!=null){
                breeding_services_list.clear();
              }
              load_list=json.decode(response);
              breeding_services_list = load_list['response'];
              total_page=load_list['totalPages'];
        ...
        child: InkWell(child: Icon(Icons.add),
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) =>breeding_service_form(token)),)
