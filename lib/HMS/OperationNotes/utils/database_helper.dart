...
class DatabaseHelper {
  ...
  String colId = 'id';
  String colDate = 'date';
  String colGeneralCategory = 'general_category';
  ...
  void _createDb(Database db, int newVersion) async {
    ...
    await db.execute('CREATE TABLE $notesTable($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colDate TEXT, '
        '$colGeneralCategory INTEGER, $colComments TEXT)');
  }
...
//		var result = await db.rawQuery('SELECT * FROM $noteTable order by $colPriority ASC');
    var result = await db.query(notesTable, orderBy: '$colGeneralCategory');
    return result;
  }
  ...
  Future<int> deleteNote(int id) async {
    var db = await this.database;
    int result = await db.rawDelete('DELETE FROM $notesTable WHERE $colId = $id');
    return result;
  }
  ...
  Future<List<Notes>> getNoteList() async {
    ...
    for (int i = 0; i < count; i++) {
      notesList.add(Notes.fromMapObject(noteMapList[i]));
    }
