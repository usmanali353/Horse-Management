...
class _vaccinationtypes_list extends State<vaccinationtypes_list>{
  ...
  var types_lists,load_list, pagelist, pageloadlist;
  int pagenum = 1;
  int total_page;
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((result){
      if(result) {
        ...
        pd.show();
        VaccinationTypesServices.vaccinationType_by_page(token, pagenum, searchQuery).then((response){
          pd.dismiss();
          if(response!=null){
            setState(() {
                    ...
                    if(load_list['hasNext'] == true && pagenum >= 1 ) {
                      Utils.check_connectivity().then((result){
                        if(result) {
                          ProgressDialog pd = ProgressDialog(context, isDismissible: true, type: ProgressDialogType.Normal);
                          pd.show();
                          VaccinationTypesServices.vaccinationType_by_page(
                ...
                children: <Widget>[
                  Slidable(
                      ...
                      child: FadeAnimation(2.0,
                        ListTile(
                          enabled: types_lists[index]['isActive'],
        ...
        child: InkWell(child: Icon(Icons.add),
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) => add_vaccinationtypes(token)),)
