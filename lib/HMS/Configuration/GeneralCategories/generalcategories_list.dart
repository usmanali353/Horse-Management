...
class _generalcategory_list extends State<generalcategory_list>{
  ...
  var generalcategory_lists, load_list, pagelist, pageloadlist;
  int pagenum = 1;
  int total_page;
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((result){
      if(result) {
        ...
        pd.show();
        GeneralCategoryServices.generalCategories_by_page(token, pagenum, searchQuery).then((response){
          pd.dismiss();
                    ...
                    if(load_list['hasNext'] == true && pagenum >= 1 ) {
                      Utils.check_connectivity().then((result){
                        if(result) {
                          ProgressDialog pd = ProgressDialog(context, isDismissible: true, type: ProgressDialogType.Normal);
                          pd.show();
                          GeneralCategoryServices.generalCategories_by_page(
        ...
        GeneralCategoryServices.generalCategories_by_page(token,pagenum,searchQuery).then((response){
          ...
          if(response!=null){
            setState(() {
              ...
              if(generalcategory_lists!=null){
                generalcategory_lists.clear();
              }
              load_list=json.decode(response);
              generalcategory_lists = load_list['response'];
              total_page=load_list['totalPages'];
        ...
        child: InkWell(child: Icon(Icons.add),
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) => add_generalcategory(token)),)
