...
import 'package:horse_management/HMS/Configuration/IronBrand/ironBrand_details.dart';
...
class _ironbrand_list extends State<ironbrand_list>{
  ...
  var ironbrand_lists, load_list, pagelist, pageloadlist;
  int pagenum = 1;
  int total_page;
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((result){
      if(result) {
        ...
        pd.show();
        IronBrandServices.ironBrand_by_page(token, pagenum, searchQuery).then((response){
          pd.dismiss();
                    ...
                    if(load_list['hasNext'] == true && pagenum >= 1 ) {
                      Utils.check_connectivity().then((result){
                        if(result) {
                          ProgressDialog pd = ProgressDialog(context, isDismissible: true, type: ProgressDialogType.Normal);
                          pd.show();
                          IronBrandServices.ironBrand_by_page(
                  ...
                  Slidable(
                      ...
                      child: FadeAnimation(2.0,
                         ...
                         leading: ironbrand_lists[index]['brandImage']!=null?Image.memory(base64.decode(ironbrand_lists[index]['brandImage'])):Text(''),
                          ...
                          onTap: (){
                             Navigator.push(context, MaterialPageRoute(builder: (context)=>ironBrand_details_page(ironbrand_lists[index])));
                          },
                        ),
                      )
                  ),
        ...
        IronBrandServices.ironBrand_by_page(token,pagenum,searchQuery).then((response){
          ...
          if(response!=null){
            setState(() {
              ...
              if(ironbrand_lists!=null){
                ironbrand_lists.clear();
              }
              load_list=json.decode(response);
              ironbrand_lists = load_list['response'];
              total_page=load_list['totalPages'];
        ...
        child: InkWell(child: Icon(Icons.add),
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) => add_ironbrand(token)),)
