...
class marking_list extends StatefulWidget{
  String token;
  marking_list(this.token);


  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _marking_list(token);
  }
}

class _marking_list extends State<marking_list>{
  String token;
  _marking_list(this.token);
  var temp=['','',''];
  ...
  var marking_lists, load_list, pagelist, pageloadlist;
  int pagenum = 1;
  int total_page;
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((result){
      if(result) {
        ...
        pd.show();
        MarkingServices.marking_by_page(token, pagenum, searchQuery).then((response){
          pd.dismiss();
          if(response!=null){
            setState(() {
    ...
    return Scaffold(
                    ...
                    if(load_list['hasNext'] == true && pagenum >= 1 ) {
                      Utils.check_connectivity().then((result){
                        if(result) {
                          ProgressDialog pd = ProgressDialog(context, isDismissible: true, type: ProgressDialogType.Normal);
                          pd.show();
                          MarkingServices.marking_by_page(
      ...
      body: RefreshIndicator(
        ...
        child: Visibility(
          visible: isVisible,
          child: Scrollbar(
            child: ListView.builder(itemCount:marking_lists!=null?marking_lists.length:temp.length,itemBuilder: (context,int index){
              return Column(
                children: <Widget>[
                  Slidable(
                      ...
                      secondaryActions: <Widget>[
                        IconSlideAction(
                          ...
                          caption: 'Update',
                          onTap: () async {
                            print(marking_lists[index]);
                            Navigator.push(context,MaterialPageRoute(builder: (context)=>update_marking(token,marking_lists[index])));
                          },
                        ),
                      ],
                      actions: <Widget>[
                        IconSlideAction(
                          ...
                          caption: 'Hide',
                          onTap: () async {
                            MarkingServices.changeMarkingsVisibility(token,marking_lists[index]['id']).then((response){
                              print(response);
                              if(response!=null){
                                Scaffold.of(context).showSnackBar(SnackBar(
        ...
        child: InkWell(child: Icon(Icons.add),
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) => add_marking(token)),)
