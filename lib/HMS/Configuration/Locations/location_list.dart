...
class _location_list extends State<location_list>{
  ...
  var location_lists, load_list, pagelist, pageloadlist;
  int pagenum = 1;
  int total_page;
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((result){
      if(result) {
        ...
        pd.show();
        LocationServices.location_by_page(token, pagenum, searchQuery).then((response){
          pd.dismiss();
          if(response!=null){
            setState(() {
                    ...
                    if(load_list['hasNext'] == true && pagenum >= 1 ) {
                      Utils.check_connectivity().then((result){
                        if(result) {
                          ProgressDialog pd = ProgressDialog(context, isDismissible: true, type: ProgressDialogType.Normal);
                          pd.show();
                          LocationServices.location_by_page(
        ...
        child: InkWell(child: Icon(Icons.add),
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) => add_location(token)),)
