...
class _colors_list extends State<colors_list>{
  ...
  var color_lists, load_list, pagelist, pageloadlist;
  int pagenum = 1;
  int total_page;
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((result){
      if(result) {
        ...
        pd.show();
        ColorsServices.colors_by_page(token, pagenum, searchQuery).then((response){
          pd.dismiss();
        ...
        child: InkWell(child: Icon(Icons.add),
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) => add_color(token)),)
